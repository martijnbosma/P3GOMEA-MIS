    ...
    def __getitem__(self, index):
        ...
        if self.augment: 
            ...
            if np.random.uniform(0,1) < 0.2:            
                image, mask = shift(image, mask, max_shift_h = 30, max_shift_v = 30)
            ...
            if np.random.uniform(0,1) < 0.3:                   
                image = adjust_brightness_tensor(image, 0.8, 1.3, variance = 0.2)
...
class DoubleImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment: 
            ...
            # if np.random.uniform(0,1) < 0.3:                   
            #     image = adjust_contrast_tensor(image, 0.5, 1.5, variance = 0.5)
...
class ImageDatasetBackup(data.Dataset):
