...
class NAS_WNet_blocks(nn.Module):
	def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
					...
					block_types=None):
		super(NAS_WNet_blocks, self).__init__()
		...
		# 	if b == 0:
		# 		blocks.append((InceptionBlock, 3))
		# 	elif b == 1:
		# 		blocks.append((InceptionBlock, 5))
		# 	elif b == 2: 
		# 		blocks.append((InceptionBlock, 7))
		# 	else:
		# 		raise NotImplementedError("No other blocks implemented")
		for b in block_types:
			if b == 0:
				blocks.append((VGGBlock, 3))
			elif b == 1:
				blocks.append((ResidualBlock, 3))
			elif b == 2: 
				blocks.append((ResidualBlock, 5))
			elif b == 3: 
				blocks.append((InceptionBlock, 3))
			elif b == 4:
				blocks.append((InceptionBlock, 5))
		...
		for d in range(len(self.network)):
			...
			block = blocks[d-1][0] if d>0 and d<len(blocks) else VGGBlock
			self.conv_kwargs['kernel_size'] = [blocks[d-1][1]]*2 if d>0 and d<len(blocks) else [3,3] #self.conv_kernel_sizes[d-1]
			self.conv_kwargs['padding'] = [blocks[d-1][1]//2]*2 if d>0 and d<len(blocks) else [1,1] #self.conv_pad_sizes[d-1]
...
class NAS_multipath_WNet_blocks(nn.Module):
	...
	def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
					...
					max_num_features=None, basic_block=ConvDropoutNormNonlin, depth=None, skip_connects=None,
					n_layers_path=10):
		super(NAS_multipath_WNet_blocks, self).__init__()
...
if __name__ == '__main__':
	device = "cpu"
	input1 = torch.rand(64, 3, 256, 256).to(device)
	conv_op = nn.Conv2d
	...
	model = NAS_WNet_blocks(3, 32, 1, 9,
